# American LIFE Kırşehir — Strateji

**Son güncelleme:** 2026-09-25

## 1. Ana hedef (3-6 ay)
Ekim-Kasım 2026 dönem başında Almanca ve çocuk/teen gruplarını doldurmak. Ölçülebilir hedef **BİLİNMİYOR**; hedef kayıt sayısı müşteriden alınacak.

## 2. Mevcut strateji
- **Soğuk trafik:** Instagram profil ziyareti (ucuz: 2-3 TL). Ürün başına ayrı ad set: Almanca / Çocuk-Teen. Kitleyi kreatif seçiyor, veli ilgileri sinyal olarak kullanılıyor.
- **Sıcak trafik (öneri):** 30 günlük profil ziyaretçisi ve etkileşim kitlesine teklifli WhatsApp mesaj reklamı.
- **Neden:** Soğuk mesaj reklamı 2026'da 117-542 TL/konuşma arasında dalgalandı ve bütçe artınca maliyet de arttı. Profil ziyareti ise 2-3 TL'de istikrarlı.

## 3. Bütçe planı
| Kampanya | Günlük | Gerekçe |
|---|---|---|
| İnstagram trafik → Almanca ad set (yeni) | 200 TL | Ajans sahibi onayı 2026-09-25 |
| İnstagram trafik → Çocuk-Teen ad set (yeni) | 200 TL | Ajans sahibi onayı 2026-09-25 |
| Yeni Yapay zeka Video - Kopya (aktif, yetişkin İngilizce) | 201 TL | Mevcut; frekans takip edilecek |
| wp Mesaj → RT Almanca (120250678417670043, PAUSED) | 100 TL | 45G İG etkileşim (120250678474230043) + 45G İG profil ziyaret (120250678474480043) + 365G %75 video (2 kitle), müşteri listesi hariç, ADV kapalı. 2 reklam (2+1, aile birleşimi) |
| wp Mesaj → RT Çocuk-Teen (120250678417950043) | 100 TL | Aynı kitleler, Kırşehir, 25-55. 3 reklam (2+1, Teenglish, 7-14). Ajans sahibi açmıştı; kitle güncellemesi Meta tarafından otomatik PAUSED'a çekildi |
| wp Mesaj 2 (aktif) | 301 TL | Mevcut; Eylül'de 336 TL/konuşma. Sıcak kitleye çevrilmesi öneriliyor |

## 4. Açık testler
| Test | Hipotez | Başlangıç | Bitiş | Başarı kriteri | Durum |
|---|---|---|---|---|---|
| Yeni profil ziyareti ad set'leri (aktif `İnstagram trafik` kampanyası içinde, Almanca + Çocuk, her birinde 5 metinli 1 reklam) | Teklifli yeni görseller 2-3 TL/ziyaret bandına döner | Görseller eklenip yayına alınınca | +7 gün | ≤ 3,0 TL/ziyaret ve CTR ≥ %1,0 | Ad set'ler kuruldu (PAUSED). Reklamlar Reklam Yöneticisi'nde eklenecek (bkz. rapor §1) |
| Sıcak kitleye WP mesaj (2 RT seti) | Profil ziyaretçisine 2+1 teklifli mesaj reklamı soğuktan ucuzdur | Açılınca (öneri: trafik reklamlarından 5-7 gün sonra) | +7 gün | ≤ 200 TL/konuşma, 7 günde ≥ 5 konuşma (set başına) | Kuruldu, PAUSED, hata yok |

*Sonuçlanmamış testi bozma. Test süresi dolmadan strateji değiştirilmez.*

## 5. Tamamlanan testler ve öğrenilenler
| Test | Sonuç | Karar |
|---|---|---|
| Veli ilgileri + Kırşehir şehir, profil ziyareti (Mar-Ağu 2026) | 2,06-2,50 TL/ziyaret | Yeni çocuk ad set'inde kopyalandı |
| Kasım 2025 - Ocak 2026 video/görseller, profil ziyareti | 10-12,5 TL/ziyaret, CTR %0,4-0,5 | Kreatif zayıftı; kitleden bağımsız |
| Sağlık ilgisi hedefli WP (Mart 2026) | 363-913 TL/konuşma | Almanca için ilgi hedefi yerine geniş hedefleme seçildi |
| "Ağustos eski tutan" WP ad set'leri | 562-947 TL/konuşma | Durduruldu |
| Bursluluk mesaj reklamı (2025) | 40 TL/konuşma | Somut teklif mesaj maliyetini düşürüyor (HYPOTHESIS) |

## 6. Bilinen kısıtlar
- Kırşehir küçük bir pazar. Veli kitlesi hızlı doyuyor, bütçe artınca frekans yükseliyor.
- Kayıt verisi yok. Meta'da sadece ara metrik (ziyaret, konuşma) ölçülüyor.
- MCP Instagram hesabını göremiyor (instagram_basic izni eksik olabilir). IG business ID kitle kuralından bulundu: 3877593725702436.
- ThruPlay/video izleyici kitlesi MCP ile oluşturulamıyor (iki kural biçimi de reddedildi). Video kitleleri Reklam Yöneticisi'nden açılmalı.

## 7. Sıradaki öncelikler
1. Profil ziyareti reklamları ajans sahibi tarafından Reklam Yöneticisi'nde yayına alındı (Almanca Trafik, Çocuklar Trafik1-3). RT mesaj setlerini 5-7 gün sonra aç (Ajans).
2. Görsel 3 ve 5'teki bozuk yazıları düzelt (Ajans).
3. 2+1 teklifinin kapsamını, geçerlilik tarihini ve seviye ücretlerini teyit et (Müşteri).
4. Haftalık kayıt sayısını kaynağa göre (IG DM / WP / telefon) paylaşsın (Müşteri).
5. 7 gün sonra: sıcak kitleye WP mesaj testini kur (Ajans, onayla).
